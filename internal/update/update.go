...
const (
	releaseAPI       = "https://api.github.com/repos/KLIXPERT-io/ga4-cli/releases/latest"
	assetURLTemplate = "https://github.com/KLIXPERT-io/ga4-cli/releases/download/%s/%s"
...
func LatestRelease(ctx context.Context, currentVersion string) (string, error) {
	...
	req.Header.Set("User-Agent", "ga4-cli/"+currentVersion)
...
func archiveAssetName(tag string) string {
	...
	return fmt.Sprintf("ga4_%s_%s_%s.%s", v, runtime.GOOS, runtime.GOARCH, ext)
}

func downloadAndSwap(ctx context.Context, tag, runningPath string) error {
	tmp, err := os.MkdirTemp(os.TempDir(), "ga4-update-*")
	if err != nil {
		return err
	}
	...
	stagedBin, err := os.CreateTemp(stagingDir, ".ga4-new-*")
	if err != nil {
		// Fall back to temp dir if target dir is not writable for temp creation.
		stagedBin, err = os.CreateTemp(tmp, ".ga4-new-*")
		if err != nil {
			return err
		}
	}
...
func debugf(format string, args ...any) {
	if os.Getenv("GA4_UPDATE_DEBUG") == "" {
		return
	}
	fmt.Fprintf(os.Stderr, "[ga4-update] "+format+"\n", args...)
}
