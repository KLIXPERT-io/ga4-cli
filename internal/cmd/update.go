...
func newUpdateCmd(version string) *cobra.Command {
	c := &cobra.Command{
		Use:   "update",
		Short: "Manage ga4 self-update (status, check, apply)",
	}
	c.AddCommand(
...
func autoUpdateStatus(cfg *config.Config, execPath string) (enabled bool, reason string) {
	if v, ok := os.LookupEnv("GA4_NO_UPDATE"); ok && v != "" {
		...
		case "0", "false":
		default:
			return false, "env:GA4_NO_UPDATE"
		}
	}
...
func newUpdateCheckCmd(version string) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				fmt.Fprintf(os.Stderr, "ga4: update check failed: %v\n", err)
			}
			data := map[string]any{
				"updated": res.Updated,
				"from":    res.From,
...
func newUpdateApplyCmd(version string) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if res.Updated {
				fmt.Fprintf(os.Stdout, "ga4: updated to %s (was %s)\n", res.To, res.From)
			} else {
				fmt.Fprintf(os.Stdout, "ga4: no update applied (%s)\n", res.Reason)
			}
			return nil
		},
	}
