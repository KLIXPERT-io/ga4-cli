...
import (
	...
	"github.com/KLIXPERT-io/ga4-cli/internal/auth"
	...
	"github.com/KLIXPERT-io/ga4-cli/internal/logging"
	"github.com/KLIXPERT-io/ga4-cli/internal/output"
	"github.com/KLIXPERT-io/ga4-cli/internal/quota"
	"github.com/KLIXPERT-io/ga4-cli/internal/update"
...
const stateKey ctxKey = "ga4.state"
...
// buildClient returns an authed API client and the caller identity.
...
func (s *State) buildHTTPClient(ctx context.Context) (*http.Client, string, error) {
	...
	if err != nil {
		hint := "Run `ga4 auth login`."
		if creds.Kind == auth.KindServiceAccount {
			hint = "Check the service account key with `ga4 auth status`; it may have been disabled or revoked."
		}
...
func (s *State) credentials() (*auth.Credentials, error) {
	...
	if err != nil {
		...
		hint := "Pass --credentials <client_secrets.json> or --service-account <key.json>, set GA4_CREDENTIALS / GA4_SERVICE_ACCOUNT / GOOGLE_APPLICATION_CREDENTIALS, or run `ga4 config set auth.credentials_path <path>`."
...
// source wins over a generic one, so a CI job can export GA4_SERVICE_ACCOUNT
...
func (s *State) resolveCredentials() (path, subject string) {
	...
	path = firstNonEmpty(
		s.SAPath,
		s.CredsPath,
		os.Getenv("GA4_SERVICE_ACCOUNT"),
		os.Getenv("GA4_CREDENTIALS"),
		os.Getenv("GOOGLE_APPLICATION_CREDENTIALS"),
		cfg.Auth.ServiceAccountPath,
		cfg.Auth.CredentialsPath,
	)
	subject = firstNonEmpty(s.Subject, os.Getenv("GA4_SUBJECT"), cfg.Auth.Subject)
...
func Execute(version string) int {
	...
	root := &cobra.Command{
		Use:   "ga4",
		Short: "Google Analytics 4 CLI — LLM-friendly, fast, cached",
		Long: `ga4 is a Go CLI over the Google Analytics Data API v1beta and Admin API v1beta.
It outputs structured JSON (default), CSV, or pretty tables, caches reads
locally, tracks the Data API token budget, and emits machine-readable errors
for LLM agents.`,
		SilenceUsage:  true,
		SilenceErrors: true,
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			st.Quota.WarnFn = func(msg string) {
				if !st.Quiet {
	...
	root.PersistentFlags().BoolVarP(&st.Verbose, "verbose", "v", false, "verbose logs to stderr")
...
// GA4_NO_UPDATE_NOTICE.
func maybePrintUpdateNotice(cmd *cobra.Command, version string) {
	if v := os.Getenv("GA4_NO_UPDATE_NOTICE"); v != "" && v != "0" && v != "false" {
		return
	}
	...
	fmt.Fprintf(cmd.ErrOrStderr(), "ga4: updated to %s (was v%s)\n", installed, version)
}
...
func emit(cmd *cobra.Command, data any, meta output.Meta, columns []string, rows []output.Row) error {
	...
	f := output.ResolveFormat(fmtStr, os.Stdout.Fd())
	switch f {
