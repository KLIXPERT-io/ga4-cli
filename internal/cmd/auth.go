...
OAuth client (client_secrets.json)
  ...
  to <config-dir>/ga4/token.json (mode 0600).
...
Service account key
  ...
  service account's client_email under GA4 Admin → Property access management
  with at least the Viewer role.

Examples:
  ga4 auth login --credentials ~/secrets/ga4-client.json
  ga4 auth login --service-account ~/secrets/ga4-sa.json
  GA4_SERVICE_ACCOUNT=~/secrets/ga4-sa.json ga4 auth login`,
			...
			if creds.Kind == auth.KindServiceAccount {
				...
				if err != nil {
					return errs.New(errs.CodeAuthDenied, err.Error()).
						WithHint("Confirm the key is still active and that the Google Analytics Data API is enabled for its project.")
				}
...
func newAuthStatusCmd() *cobra.Command {
	return &cobra.Command{
		...
		Long: `Reports which credentials are active, whether they currently yield a valid
access token, and the identity they authenticate as — the OAuth client_id, or
the service account's client_email.

Examples:
  ga4 auth status
  ga4 auth status --output json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			s := getState(cmd)
			creds, err := s.credentials()
			if err != nil {
			...
			if err != nil {
				return errs.New(errs.CodeAuthMissing, err.Error()).WithHint("Run `ga4 auth login`.")
			}
