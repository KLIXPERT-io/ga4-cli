...
// Grant the service account access by adding its client_email as a user with
// at least Viewer role on the GA4 property (Admin → Property access
// management), or use domain-wide delegation with a subject.
package auth
...
// that are useful for `ga4 auth status` and cache keying.
