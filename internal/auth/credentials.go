...
// OAuth client (interactive `ga4 auth login`) and a service account key file.
package auth
...
const (
	...
	// requires `ga4 auth login` and a stored user token.
...
var ErrNoCredentials = errors.New("no credentials configured: pass --credentials or --service-account, set GA4_CREDENTIALS / GA4_SERVICE_ACCOUNT / GOOGLE_APPLICATION_CREDENTIALS, or run `ga4 config set auth.credentials_path <path>`")
...
// --credentials/GA4_CREDENTIALS setting accepts either flavor.
...
// `ga4 auth login` before they can be used.
